...
class Week52HighPage extends StatefulWidget {
  Week52HighPage({Key key}) : super(key: key);

  @override
  _Week52HighPageState createState() => _Week52HighPageState();
}

class _Week52HighPageState extends State<Week52HighPage>
  ...
  _fetchApi() async {
    stockData = await StockServices2.get52WeekHigh();
    // stockData.bse  =stoc
    setState(() {
      loading = false;
    });
            ...
            return loading
                ...
                : !stockData.nse.isEmpty || stockData != null
                    ? TabBarView(controller: _controller, children: [
                        section(context, stockData.bse),
                        section(context, stockData.nse)
                      ])
                    : NoDataAvailablePage();
          },
        ),
              ...
              children: List.generate(
                  ...
                  (index) => InkWell(
                        ...
                        child: CustomCardStock(
                          ...
                          price: d[index].price,
                          highlight:
                              (double.parse(d[index].chg.replaceAll(',', '')) >
                                          0
                                      ? "${d[index].chg}"
                                      : "${d[index].chg}") +
                                  " (${d[index].chgPer})",
